...
1. **The recognizer identifies; the database prices.** The vision model returns only item identity, quantity/instance, confidence, and optional evidence such as bounding boxes and uncertainty flags. The app applies prices from the local catalog and computes the total. The model never sees or sets prices.
...
**Architecture seam:** Recognition sits behind a single `Recognizer` interface (image + catalog in → `[{sku?, qty, confidence, bbox?, occluded?, possiblyMore?, alternates?}]` out). Phase 1 implements it with a cloud vision LLM; Phase 2 adds an on-device implementation to drive per-scan cost toward zero. The vendor is a swappable config choice — never hardcoded in domain or UI.
...
#### Module 2 — Scan
*Photograph the counter, recognize catalog items, and build an editable, priced draft receipt. Recognition output is identity + quantity/instance + confidence, with optional bounding-box and uncertainty evidence; pricing is deterministic.*
...
- **Catalog context strategy:** Per scan, send the active catalog as a compact `{sku, name}` text list, with optional per-SKU visual descriptions and look-alike group hints when catalog metadata is populated. For catalogs at or under the R1 cap (30 SKUs), also attach one labeled enrolled-reference thumbnail per SKU before the counter photo so the model can match against actual packaging. The cloud prompt asks for one entry per visible physical item instance with an optional normalized box plus occlusion, possible-hidden-count, and look-alike alternate fields; pricing groups those instances back to one draft line per SKU, routes null SKUs to unidentified items, and force-flags identified lines in active look-alike groups for cashier review. When the cloud recognizer is configured, uncertain boxed detections are cropped from the retained full-resolution source and verified against a narrowed active sub-catalog; confident scans still use only the primary recognition pass. Above the reference-photo cap, fall back to the text-only request; pre-narrowing remains the at-scale path.
...
- `Recognizer` returns identity + quantity/instance + confidence, optionally with normalized bounding-box and uncertainty evidence — never price.
