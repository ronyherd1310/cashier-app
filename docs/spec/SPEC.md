...
- **Catalog context strategy:** Per scan, send the active catalog as a compact `{sku, name}` text list. For catalogs at or under the R1 cap (30 SKUs), also attach one labeled enrolled-reference thumbnail per SKU before the counter photo so the model can match against actual packaging. The cloud prompt asks for one entry per visible physical item instance with an optional normalized box plus occlusion, possible-hidden-count, and look-alike alternate fields; pricing groups those instances back to one draft line per SKU and routes null SKUs to unidentified items. Above the cap, fall back to the text-only request; pre-narrowing remains the at-scale path.
